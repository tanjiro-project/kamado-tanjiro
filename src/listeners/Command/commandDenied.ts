...
export class clientListener extends Listener {
    run({ context, message: content }: UserError, { message }: CommandDeniedPayload) {
        ...
        if (!(message.channel as TextChannel).permissionsFor(message.guild?.me!).has("SEND_MESSAGES")) return message.author.send({ embeds: [new MessageEmbed().setDescription(`❌ | I dont have send message permissions in ${message.channel.toString()}`).setColor(botEmbedColor)], allowedMentions: { users: [message.author.id], roles: [] } }).catch(e => this.container.logger.error("PROMISE_ERR:", e));
