...
@ApplyOptions<CommandOptions>({
    name: "lock",
    description: "Lock your temporary voice session",
...
export class ClientCommand extends Command {
    async messageRun(message: Message) {
        ...
        await voiceChannel.edit({ userLimit: voiceChannel.members.size });

        return message.reply({
            embeds: [
                new MessageEmbed()
                    .setColor(botEmbedColor)
                    .setDescription("👌 | Locked you current voice session")
            ]
        });
    }
}
